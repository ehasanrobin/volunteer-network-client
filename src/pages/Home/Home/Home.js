...
const Home = () => {
    ...
    const match = services.filter(service => service.name.includes(text));
     setsearchEvents(match);
    }
    return (
        <section className='home-section'>
           <Container className='text-center'>
            ...
            <div className=''>
                <Row xs={1} md={2} xl={4}>
                     {
                searchEvents.map(service => <Service key={service._id} service={service}></Service>)
            }
                </Row>
            </div>
            </Container>
